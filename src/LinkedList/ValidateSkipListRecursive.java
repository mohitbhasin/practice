...
class ValidateSkipListRecursive {
	...
	public static boolean validate(ListNode head) {
		...
		return checkCycle(head, new HashSet<>());
	}

	public static boolean checkCycle(ListNode curr, HashSet<ListNode> visited) {
		if(curr==null) {
			return true;
		}
		if(visited.contains(curr)) return false;
		visited.add(curr);
		if(!checkCycle(curr.next, visited)) return false;
		visited.remove(curr);
		return checkCycle(curr.skip, visited);
	}
}
